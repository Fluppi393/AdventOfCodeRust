    ...
    inline auto solve(const bool is_test)
    {
        ...
        auto result_p1 = 0ull;
        {
            ...
            auto column_results = std::views::iota(0ull, num_columns) | std::views::transform([&](const auto column)
            {
                ...
                {
                    return numbers[line_idx][column];
                });
        ...
        auto result_p2 = 0ull;
        {
            ...
            for (auto op_it = ops_line.begin(); op_it < ops_line.end();)
            {
                ...
                for (auto column_idx = 0ull; column_idx < num_columns; ++column_idx)
                {
                    ...
                    auto number_range = number_lines | std::views::transform([&](const auto& s) { return s[column_idx + column_offset]; }) | std::views::filter([](const char c)
                    {
                        return !std::isspace(c);
                    });

                    // Convert to a string and then an integer
                    const auto number_str = std::string(number_range.begin(), number_range.end());
                    numbers.push_back(std::stoull(number_str));
                }
